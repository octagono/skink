...
- **WSS transport served through HTTP/HTTPS proxy, NOT the control port** — `server.go` no longer starts a separate WSS HTTP server on `s.port` (it conflicts with the TCP control listener — Go cannot bind two listeners on the same port). WSS upgrade is registered on the HTTP proxy (`--tunnel-http-port`, default 8080) via `SetWSSHandler` in `cli.go`. The client's WSS dialer (`transport_ws.go` `utlsWSSDialer`) connects with TLS then WebSocket upgrade to `ws://host:8080/tunnel`. Without TLS configured, WSS is unavailable (client always uses utls).
