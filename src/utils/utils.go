...
func GetConfigDir(requireValidPath bool) (homedir string, err error) {
	if envHomedir, isSet := os.LookupEnv("SKINK_CONFIG_DIR"); isSet {
		homedir = envHomedir
	} else if xdgConfigHome, isSet := os.LookupEnv("XDG_CONFIG_HOME"); isSet {
		homedir = path.Join(xdgConfigHome, "Skink")
	} else {
		...
		homedir = path.Join(homedir, ".config", "Skink")
	}
...
func ZipDirectory(destination string, source string, ignoredPaths map[string]bool, exclusions []string) (err error) {
	...
	// no compression because Skink does its compression on the fly
...
const crocRemovalFile = "Skink-marked-files.txt"
