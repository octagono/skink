...
```bash
go build -o ./bin/Skink && ./bin/Skink relay --ports 9009,9010
```

In the second terminal, start the receiver:

```bash
rm -rf Skink-big.bin && ./bin/Skink --debug --yes --overwrite --relay 127.0.0.1:9009 test-reconnect-code
```

In the third terminal, start the sender:

```bash
./bin/Skink --relay 127.0.0.1:9009 --throttleUpload 512K --no-compress send --code test-reconnect-code --no-local --no-multi /tmp/Skink-big.bin
```
...
If no connection is killed, inspect the active Skink sockets and adjust the port:

```bash
sudo ss -tnp | grep Skink
```
