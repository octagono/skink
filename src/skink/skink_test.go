...
func TestSkinkReadme(t *testing.T) {
	defer os.Remove("README.md")

	log.Debug("setting up sender")
	sender, err := New(Options{
		IsSender:      true,
		SharedSecret:  "8123-testingtheskink",
		Debug:         true,
...
func TestSkinkEmptyFolder(t *testing.T) {
	pathName := "../../testEmpty"
...
func TestSkinkSymlink(t *testing.T) {
	...
	sender, err := New(Options{
		IsSender:      true,
		SharedSecret:  "8124-testingtheskink",
		Debug:         true,
		RelayAddress:  "127.0.0.1:8281",
		RelayPorts:    []string{"8281"},
		RelayPassword: "pass123",
...
func TestSkinkIgnoreGit(t *testing.T) {
	log.SetLevel("trace")
	...
	filesInfo, _, _, errGet := GetFilesInfo([]string{"../../LICENSE", ".gitignore", "skink.go"}, false, true, []string{})
	if errGet != nil {
		t.Errorf("failed to get minimal info: %v", errGet)
	}
...
// https://github.com/octagono/skink/issues/1087: --exclude and --git were
...
func TestSkinkLocal(t *testing.T) {
	log.SetLevel("trace")
...
func TestSkinkError(t *testing.T) {
	...
	sender, _ := New(Options{
		IsSender:      true,
		SharedSecret:  "8123-testingtheskink2",
		Debug:         true,
...
func TestReceiverStdoutWithInvalidSecret(t *testing.T) {
	// Test for issue: panic when receiving with --stdout and invalid SKINK_SECRET
